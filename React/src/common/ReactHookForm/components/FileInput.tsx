...
const FileInput: FC<FileInputProps> = ({className, controller, uploadOnChange = true, multiple = false, ...props}) => {

    const [progress, setProgress] = useCallAPIState<number>(
        {
    ...
    const onFileChange = useCallback(
        async (event: ChangeEvent<HTMLInputElement>) => {
            ...
            const value = multiple ? [...files!] : files!.item(0)
                ...
                interceptor.post(
                    ...
                    formData,
                    {
                        ...
                        onUploadProgress: ({progress}) => {
                            setProgress(
                                progress === 1 ? CALL_API_STATUS.SUCCESS : CALL_API_STATUS.LOADING,
                                progress ?? 0
                            )
                        },
                    }
            ...
            if (success) {
                setValue(
                    name,
                    multiple ? payload?.map(
                        ({filename}) => filename
                    ) : payload?.at(-1)?.filename
                )
            }

        },
        [multiple, uploadOnChange, setValue, name, setProgress]
    )


    return (
        <label
            className={cx("relative flex items-center gap-x-2 border-2 rounded focus:border-gemini-accent", className)}
            {...props}
        >
            {
                progress.loading ? (
                    <div className="absolute bottom-0 bg-slate-100 left-0 right-0">
                        <div
                            style={
                                {width: `${progress.data * 100}%`}
                            }
                            className="h-1 bg-gemini-accent rounded animate-pulse">
                        </div>
                    </div>
                ) : null
            }
